    ...
    if (algorithm == A_STAR)
    {
        ...
        while (open_list.size() || current_parent == destination)
        {
            ...
            while (i < current_parent->total_slots())
            {
                ...
                Road*       road    = current_parent->slot(i)->road();
                ...
                &&  !binary_search(closed_list.begin(), closed_list.end(), child))
                {
                    ...
                    unsigned short j(0);
                    while (j < open_list.size() && open_list[j]->f_cost() < child->f_cost())
                    {
                        j++;
                    }
                ...
                {
                    float current_g_cost = current_parent->g_cost() + current_road->weight();
